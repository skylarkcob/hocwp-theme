...
add_action( 'hocwp_theme_widget_before', 'hocwp_theme_widget_before', 10, 3 );
...
function hocwp_theme_dynamic_sidebar_params_filter( $params ) {
	if ( isset( $params[0] ) ) {
		...
		if ( isset( $args['before_widget'] ) ) {
			...
			$params[0]['before_widget'] = '<section class="widget"><div class="widget-inner">';
		}
		...
		if ( isset( $args['before_title'] ) ) {
			...
			$params[0]['before_title'] = '</div>' . $before_title;
		} else {
			$params[0]['before_title'] = '</div><h3 class="widget-title widgettitle">';
		}
		...
		if ( isset( $args['after_title'] ) ) {
			...
			$params[0]['after_title'] = '</h3><div class="widget-content">';
		}
...
function _hocwp_theme_widget_posts_loop() {
	global $hocwp_theme;
		...
		while ( $query->have_posts() ) {
			...
			if ( $template_valid ) {
				...
				if ( $on_sidebar && is_array( $instance ) ) {
					_hocwp_theme_widget_posts_loop();
				} else {
...
function hocwp_theme_socials( $args = array() ) {
    ...
    <div class="social share-tools">
		...
		if ( current_user_can( 'publish_posts' ) ) {
			...
			?>
            <a href="<?php echo esc_url( $base ); ?>" rel="nofollow" target="_blank" class="<?php echo $class; ?>"
               title="<?php echo esc_attr( __( 'Submit URL to Google Search Console', 'hocwp-theme' ) ); ?>"><?php echo $name; ?></a>
			<?php
		}
		?>
    </div>
...
function hocwp_theme_pre_get_posts_action( $query ) {
	if ( $query instanceof WP_Query && $query->is_main_query() ) {
		if ( is_home() ) {
			$ppp = hocwp_theme_get_option( 'posts_per_page', '', 'home' );
			if ( ! is_numeric( $ppp ) ) {
				$ppp = $GLOBALS['hocwp_theme']->defaults['posts_per_page'];
			}
